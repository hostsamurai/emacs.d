;;;###autoload
(defun hostsamurai/read-file-to-string (path)
...
;;;###autoload
(defun hostsamurai/find-subpath-in-path (subpath path)
  ...
  (let ((match (concat (file-name-as-directory path) subpath)))
    (if (file-exists-p match)
        match
      (unless (string= path "/")
        (hostsamurai/find-subpath-in-path
         subpath
         (file-name-directory (substring path 0 -1)))))))

;;;###autoload
(defun hostsamurai/find-in-path (subpath)
